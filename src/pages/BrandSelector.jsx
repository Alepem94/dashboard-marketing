...
export function BrandSelector() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        // Cargar empresa y marcas en paralelo
        const [empresaRes, configRes] = await Promise.all([
          fetch(getSheetURL('_EMPRESA')),
          fetch(getSheetURL('_CONFIG'))
        ])
        
        const empresaCsv = await empresaRes.text()
        const configCsv = await configRes.text()
        
        // Parsear empresa
        const { data: empresaData } = Papa.parse(empresaCsv, { header: true, skipEmptyLines: true })
        const empresaObj = {}
        empresaData.forEach(row => {
        ...
        setBrands(configData.filter(row => row.marca_id && row.nombre))
